...
public class JwtUtils {
	...
	/*
	 * Genera il token con: dati dell'utente, data di scadenza, algoritmo di
	...
	public String generateJwtToken(Authentication authentication) {
		...
		UtenteDetailsImpl userPrincipal = (UtenteDetailsImpl) authentication.getPrincipal();
		Date now = new Date();
		Date exp = new Date((now).getTime() + jwtExpirationMs);
		userPrincipal.setExpirationTime(exp);
		// Usa il builder di Jwts per generare il token
		return Jwts.builder().setSubject((userPrincipal.getEmail())).setIssuedAt(now).setExpiration(exp)
